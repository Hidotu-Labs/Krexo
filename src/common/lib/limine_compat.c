...
void limine_handle_requests(void *kernel_start, size_t kernel_size,
  ...
  for (size_t i = 0; i < count - 3; i++) {
    ...
    if (i < count - 1 && ptr[i] == LIMINE_END_0 && ptr[i + 1] == LIMINE_END_1) {
      scan_end = i;
  ...
  for (size_t i = scan_start; i < scan_end; i++) {
    ...
    // --- Firmware Type ---
    else if (req_id0 == LIMINE_FIRMWARE_ID0 && req_id1 == LIMINE_FIRMWARE_ID1) {
      typedef struct {
        uint64_t revision;
    ...
    else if (req_id0 == LIMINE_CMDLINE_ID0 && req_id1 == LIMINE_CMDLINE_ID1) {
      typedef struct {
        uint64_t revision;
        uint64_t cmdline;
      } cmdline_resp_t;
