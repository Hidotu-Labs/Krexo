...
void krexo_fb_draw_noise_bg_rect(krexo_fb_t *fb, uint32_t x_start, uint32_t y_start, uint32_t w, uint32_t h, uint32_t base_color) {
  ...
  if (fb->bpp != 32) {
    ...
    for (uint32_t y = y_start; y < y_start + h && y < fb->height; y++) {
      for (uint32_t x = x_start; x < x_start + w && x < fb->width; x++) {
        ...
        uint8_t noise = (hash >> 13) & 0x7;
        uint32_t color = ((r_base > noise ? r_base - noise : 0) << 16) |
                         ((g_base > noise ? g_base - noise : 0) << 8)  |
                         (b_base > noise ? b_base - noise : 0);
        krexo_fb_put_pixel(fb, x, y, color);
      }
    }
    return;
  ...
  for (uint32_t y = y_start; y < y_start + h && y < fb->height; y++) {
    uint32_t *pixel_ptr = (uint32_t *)((uint8_t *)fb->base + y * fb->pitch + x_start * 4);
    uint32_t y_grad_r = (r_base * (fb->height - y)) / fb->height;
    uint32_t y_grad_g = (g_base * (fb->height - y)) / fb->height;
    uint32_t y_grad_b = (b_base * (fb->height - y)) / fb->height;

    for (uint32_t x = x_start; x < x_start + w && x < fb->width; x++) {
      uint32_t hash = (x * 1597463007U) ^ (y * 2731467445U);
      uint8_t noise = (hash >> 13) & 0x7;
      
      uint32_t r = (y_grad_r > noise) ? y_grad_r - noise : 0;
      uint32_t g = (y_grad_g > noise) ? y_grad_g - noise : 0;
      uint32_t b = (y_grad_b > noise) ? y_grad_b - noise : 0;

      if (fb->format == KREXO_FB_FORMAT_RGBX_8888) {
        *pixel_ptr++ = (r << 16) | (g << 8) | b;
      } else {
        *pixel_ptr++ = (b << 16) | (g << 8) | r;
      }
    }
  }
}
...
void krexo_fb_draw_noise_bg(krexo_fb_t *fb, uint32_t base_color) {
  krexo_fb_draw_noise_bg_rect(fb, 0, 0, fb->width, fb->height, base_color);
}
