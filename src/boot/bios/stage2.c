...
void setup_long_mode_paging() {
  ...
  // PML4 index for 0xffff800000000000 = (0xffff800000000000 >> 39) & 0x1FF =
  // 256
  ...
  if (bios_disk_init(&disk, (uint8_t)boot_drive) == 0) {
    ...
    if (fat32_init(&disk) == 0) {
      ...
      if (config.background_image_path[0]) {
        ...
        if (fat32_find_file(config.background_image_path, &bg_cluster,
                            &bg_size) == 0) {
